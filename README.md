...
It also has a nifty system for easy localisation, a flexible save game system, loads of helpful math functions and more!
...
Statemachines are serialised to text assets (as xml) and support cross scene refs for gameobjects/components.

It supports timeline states which can trigger events (and its easy to write/add your own), coroutine states that simply run and wait for a coroutine on a monobehaviour, and conditional states which will make the statemachine remain in the state until a condition is met. 
These can be used to branch logic or wait for an player action.
...
The component that runs the nodegraph can however pass it data values unique to itself via input nodes.
This means for example the same node graph that idly oscillates an object (eg a floating rock) can be reused on different objects each with different values for oscillation height and speed 
- you dont have to change the node graphs, instead simply use different input values on the component.
