				...
				protected void Render(Camera camera)
				{
					...
					if (numAlive > 0)
					{
						...
						if (_renderedParticles.Count > 0)
						{
							...
							{
								UnityEngine.Graphics.DrawMeshInstanced(_mesh, i, _materials[i], _particleTransforms, _renderedParticles.Count, _propertyBlock, _shadowCastingMode, _recieveShadows, this.gameObject.layer);
							}
						}
					}
				}
